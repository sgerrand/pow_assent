defmodule PowAssent.Test.RepoMock do
  ...
  def one(query, _opts) do
    case inspect(query) =~ "left_join: u1 in assoc(u0, :user)" and
           inspect(query) =~
             "where: u0.provider == ^\"test_provider\" and u0.uid == ^\"existing_user\"" do
      true -> %User{id: 1, email: "test@example.com"}
      false -> nil
    end
  end

  def get_by(UserIdentity, [user_id: 1, provider: "test_provider", uid: "existing_user"], _opts),
    do: %UserIdentity{user_id: 1, provider: "test_provider", uid: "existing_user"}

  def get_by(UserIdentity, [user_id: 1, provider: "test_provider", uid: "new_identity"], _opts),
    do: nil

  def get_by(UserIdentity, [user_id: 1, provider: "test_provider", uid: "identity_taken"], _opts),
    do: nil
  ...
  def insert(%{changes: %{provider: "test_provider", uid: "identity_taken"}} = changeset, _opts) do
    changeset =
      Ecto.Changeset.add_error(changeset, :uid_provider, "has already been taken",
        constraint: :unique,
        constraint_name: "user_identities_uid_provider_index"
      )
  ...
  def insert(
        %{
          ...
          changes: %{
            user_identities: [
              %{changes: %{provider: "test_provider", uid: "identity_taken"}} =
    ...
    user_identity_changeset =
      Ecto.Changeset.add_error(user_identity_changeset, :uid_provider, "has already been taken",
        constraint: :unique,
        constraint_name: "user_identities_uid_provider_index"
      )
  ...
  def preload(%User{id: :multiple_identities} = user, :user_identities, force: true),
    do: %{
      user
      | user_identities: [
  ...
  def preload(user, :user_identities, force: true),
    do: %{user | user_identities: [%UserIdentity{id: 1, provider: "test_provider"}]}
