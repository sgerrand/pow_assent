defmodule PowAssent.Ecto.Schema do
  ...
  @callback user_identity_changeset(Schema.t() | Changeset.t(), Schema.t(), map(), map() | nil) ::
              Changeset.t()
  ...
  defmacro __using__(_config) do
    quote do
      ...
      @spec user_identity_changeset(Schema.t() | Changeset.t(), Schema.t(), map(), map() | nil) ::
              Changeset.t()
      def user_identity_changeset(user_or_changeset, user_identity, attrs, user_id_attrs),
        do:
      ...
      @spec pow_assent_user_identity_changeset(
              Schema.t() | Changeset.t(),
              Schema.t(),
              map(),
              map() | nil
            ) :: Changeset.t()
      def pow_assent_user_identity_changeset(
            user_or_changeset,
            user_identity,
            attrs,
            user_id_attrs
          ) do
        unquote(__MODULE__).changeset(
          user_or_changeset,
          user_identity,
          attrs,
          user_id_attrs,
  ...
  defmacro __has_many__ do
    quote do
      @pow_assocs {:has_many, :user_identities,
                   unquote(__MODULE__).__user_identities_module__(__MODULE__),
                   foreign_key: :user_id, on_delete: :delete_all}
    end
  end
  ...
  """
  @spec changeset(Schema.t() | Changeset.t(), Schema.t(), map(), map() | nil, Config.t()) ::
          Changeset.t()
  ...
  defp maybe_accept_invitation(%Changeset{
         data: %user_mod{invitation_token: token, invitation_accepted_at: nil} = changeset
       })
  ...
  defp user_id_field_changeset(changeset, attrs, nil),
    do: changeset.data.__struct__.pow_user_id_field_changeset(changeset, attrs)

  defp user_id_field_changeset(changeset, _attrs, user_id_attrs),
    do: changeset.data.__struct__.pow_user_id_field_changeset(changeset, user_id_attrs)

  defp maybe_email_confirmation_changeset(
         %Changeset{data: %{unconfirmed_email: _any}} = changeset,
         attrs
       ) do
