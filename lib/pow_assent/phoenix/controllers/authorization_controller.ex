defmodule PowAssent.Phoenix.AuthorizationController do
  ...
  @spec process_callback(Conn.t(), map()) ::
          {:ok, Conn.t()} | {:error, Conn.t()} | {:error, {atom(), map()} | map(), Conn.t()}
  def process_callback(conn, %{"provider" => provider} = params) do
    Plug.callback_upsert(conn, provider, params, conn.assigns.callback_url)
  end

  @spec respond_callback(
          {:ok, Conn.t()}
          | {:error, Conn.t()}
          | {:error, {atom(), map()} | map(), Conn.t()}
        ) :: Conn.t()
  def respond_callback(
        {:ok, %{private: %{pow_assent_callback_state: {:ok, :create_user}}} = conn}
      ) do
  ...
  def respond_callback(
        {:error,
         %{
           private: %{
             pow_assent_callback_state: {:error, :strategy},
             pow_assent_callback_error: error
           }
         } = conn}
      ),
  ...
  def respond_callback(
        {:error,
         %{private: %{pow_assent_callback_error: {:bound_to_different_user, _changeset}}} = conn}
      ) do
    conn
  ...
  def respond_callback(
        {:error,
         %{private: %{pow_assent_callback_error: {:invalid_user_id_field, changeset}}} = conn}
      ) do
    trigger_registration_email_confirmation_controller_callback(conn, fn conn ->
      ...
      |> redirect(
        to: routes(conn).path_for(conn, RegistrationController, :add_user_id, [provider])
      )
    end)
  end
  ...
  def respond_callback(
        {:error,
         %{
           private: %{
             pow_assent_callback_state: {:error, :create_user},
             pow_assent_callback_error: changeset
           }
         } = conn}
      ) do
  ...
  defp trigger_registration_email_confirmation_controller_callback(conn, callback) do
    ...
    cond do
      ...
      extension_enabled?(config, PowEmailConfirmation) ->
        ...
        |> EmailConfirmationCallbacks.before_respond(
          :create,
          to_email_confirmation_res(conn),
          config
        )
        |> case do
  ...
  defp to_email_confirmation_res(
         %{
           private: %{
             pow_assent_callback_state: {:error, _method},
             pow_assent_callback_error: {_type, changeset}
           }
         } = conn
       ) do
  ...
  def respond_delete({:error, {:no_password, _changeset}, conn}) do
    ...
    |> put_flash(
      :error,
      extension_messages(conn).identity_cannot_be_removed_missing_user_password(conn)
    )
    |> redirect(to: after_delete_path(conn))
  end
  ...
  defp assign_callback_url(conn, _opts) do
    url =
      routes(conn).url_for(conn, AuthorizationController, :callback, [conn.params["provider"]])
  ...
  defp assign_request_path(
         %{private: %{pow_assent_session: %{request_path: request_path}}} = conn,
         _opts
       ) do
    conn
  ...
  defp load_session_params(
         %{private: %{pow_assent_session: %{session_params: params}}} = conn,
         _opts
       ) do
    conn
  ...
  defp set_registration_option(%{private: %{pow_assent_registration: _any}} = conn, _opts),
    do: conn

  defp set_registration_option(conn, _opts),
    do: Conn.put_private(conn, :pow_assent_registration, registration_path?(conn))
  ...
  defp load_user_by_invitation_token(
         %{private: %{pow_assent_session: %{invitation_token: token}}} = conn,
         _opts
       ) do
  ...
  defp log_strategy_error(error) when is_exception(error),
    do: log_strategy_error(Exception.message(error))
